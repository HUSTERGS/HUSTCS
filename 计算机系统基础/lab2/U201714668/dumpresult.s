...
08048c77 <func4>:
 8048c77:	56                   	push   %esi //仅仅是保存数据
 8048c78:	53                   	push   %ebx // 仅仅是保存数据
 8048c79:	83 ec 04             	sub    $0x4,%esp 
 8048c7c:	8b 4c 24 10          	mov    0x10(%esp),%ecx // 第一个参数
 8048c80:	8b 44 24 14          	mov    0x14(%esp),%eax // 第一次传的是0
 8048c84:	8b 5c 24 18          	mov    0x18(%esp),%ebx // 第一次传的是14
 ...
 8048c99:	7f 0f                	jg     8048caa <func4+0x33> // 如果ecx比edx小
 8048c9b:	b8 00 00 00 00       	mov    $0x0,%eax
 8048ca0:	39 ca                	cmp    %ecx,%edx
 8048ca2:	7c 1b                	jl     8048cbf <func4+0x48> // edx < ecx
 ...
 8048cd0:	8d 44 00 01          	lea    0x1(%eax,%eax,1),%eax // eax = 2 * eax + 1 
...
08048cd6 <phase_4>:
 ...
 8048d00:	83 f8 02             	cmp    $0x2,%eax // eax == 2
 8048d03:	74 33                	je     8048d38 <phase_4+0x62>
 8048d05:	e8 f9 03 00 00       	call   8049103 <explode_bomb>
 8048d0a:	83 ec 04             	sub    $0x4,%esp // esp - 4 
 8048d0d:	6a 0e                	push   $0xe // 14
 8048d0f:	6a 00                	push   $0x0 // 0
 8048d11:	ff 74 24 10          	pushl  0x10(%esp) // 第一个参数
 8048d15:	e8 5d ff ff ff       	call   8048c77 <func4>
 8048d1a:	83 c4 10             	add    $0x10,%esp // 加了之后 esp + 4 等于第一个参数, esp + 8等于第二个参数
 ...
 8048d37:	c3                   	ret    
 8048d38:	83 7c 24 04 0e       	cmpl   $0xe,0x4(%esp) // 第一个参数
 8048d3d:	76 cb                	jbe    8048d0a <phase_4+0x34> // 如果小于14
 8048d3f:	eb c4                	jmp    8048d05 <phase_4+0x2f> // 否则爆炸
 8048d41:	83 7c 24 08 04       	cmpl   $0x4,0x8(%esp) // 第二个参数
 8048d46:	75 da                	jne    8048d22 <phase_4+0x4c>  // 如果不等于4就爆炸
...
08048d4f <phase_5>:
 ...
 8048d7e:	8b 44 24 04          	mov    0x4(%esp),%eax // 第一个参数
 8048d82:	83 e0 0f             	and    $0xf,%eax  // 与15与
 8048d85:	89 44 24 04          	mov    %eax,0x4(%esp) 
 8048d89:	83 f8 0f             	cmp    $0xf,%eax // eax 小于15
 ...
 8048d9b:	8b 04 85 a0 a0 04 08 	mov    0x804a0a0(,%eax,4),%eax // 很奇怪的一个地址
 8048da2:	01 c1                	add    %eax,%ecx // ecx += eax
 8048da4:	83 f8 0f             	cmp    $0xf,%eax
 8048da7:	75 ef                	jne    8048d98 <phase_5+0x49> // 不相等就继续
 8048da9:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp) // 相等
 8048db0:	00 
 8048db1:	83 fa 0f             	cmp    $0xf,%edx // edx 需要等于15,否则就爆炸
 8048db4:	75 06                	jne    8048dbc <phase_5+0x6d>
 8048db6:	3b 4c 24 08          	cmp    0x8(%esp),%ecx // ecx 需要等于esp,否则爆炸
...
08048dde <phase_6>:
 ...
 8048e07:	83 c6 01             	add    $0x1,%esi // esi++
 8048e0a:	83 fe 06             	cmp    $0x6,%esi // 如果esi == 6 跳转
 8048e0d:	74 2e                	je     8048e3d <phase_6+0x5f>
 8048e0f:	89 f3                	mov    %esi,%ebx // ebx = esi
 8048e11:	8b 44 9c 0c          	mov    0xc(%esp,%ebx,4),%eax // eax 是下一个参数
 8048e15:	39 44 b4 08          	cmp    %eax,0x8(%esp,%esi,4) // 如果和前一个参数相等,就爆炸
 8048e19:	74 1b                	je     8048e36 <phase_6+0x58> 
 8048e1b:	83 c3 01             	add    $0x1,%ebx
 8048e1e:	83 fb 05             	cmp    $0x5,%ebx // ebx <= 5
 8048e21:	7e ee                	jle    8048e11 <phase_6+0x33>
 8048e23:	8b 44 b4 0c          	mov    0xc(%esp,%esi,4),%eax // 一开始是第一个数字
 8048e27:	83 e8 01             	sub    $0x1,%eax // eax--
 8048e2a:	83 f8 05             	cmp    $0x5,%eax // eax <= 5 否则爆炸
 ...
 8048e3d:	bb 00 00 00 00       	mov    $0x0,%ebx // 在此之前的都是在确认小于等于6并且各不相等
 8048e42:	89 de                	mov    %ebx,%esi // ebx, esi 清零
 8048e44:	8b 4c 9c 0c          	mov    0xc(%esp,%ebx,4),%ecx // 遍历每一个参数
 8048e48:	b8 01 00 00 00       	mov    $0x1,%eax 
 8048e4d:	ba 3c c1 04 08       	mov    $0x804c13c,%edx
 8048e52:	83 f9 01             	cmp    $0x1,%ecx // 当前参数如果小于等于1
 8048e55:	7e 0a                	jle    8048e61 <phase_6+0x83>
 8048e57:	8b 52 08             	mov    0x8(%edx),%edx // 很奇怪的地址
 8048e5a:	83 c0 01             	add    $0x1,%eax // eax++
 8048e5d:	39 c8                	cmp    %ecx,%eax // 如果参数不等于eax, 相当于找到元素的数值对应的的位置
 8048e5f:	75 f6                	jne    8048e57 <phase_6+0x79>
 8048e61:	89 54 b4 24          	mov    %edx,0x24(%esp,%esi,4) // esi实际上在通过ebx增加,并指示当前的数字的序号，把数值对应的位置放到序号对应的位置上
 8048e65:	83 c3 01             	add    $0x1,%ebx
 8048e68:	83 fb 06             	cmp    $0x6,%ebx // 遍历每一个数字
 8048e6b:	75 d5                	jne    8048e42 <phase_6+0x64>
 8048e6d:	8b 5c 24 24          	mov    0x24(%esp),%ebx // 第一个参数对应的
 8048e71:	89 d9                	mov    %ebx,%ecx
 8048e73:	b8 01 00 00 00       	mov    $0x1,%eax
 8048e78:	8b 54 84 24          	mov    0x24(%esp,%eax,4),%edx //第二个参数对应的
 8048e7c:	89 51 08             	mov    %edx,0x8(%ecx)
 8048e7f:	83 c0 01             	add    $0x1,%eax
 8048e82:	89 d1                	mov    %edx,%ecx
 8048e84:	83 f8 06             	cmp    $0x6,%eax // 遍历
 ...
 8048e9a:	83 ee 01             	sub    $0x1,%esi // 遍历
...
08048ec7 <fun7>:
 ...
 8048ecb:	8b 54 24 10          	mov    0x10(%esp),%edx // 调用时push的地址, 最开始为36
 8048ecf:	8b 4c 24 14          	mov    0x14(%esp),%ecx // 最初的eax
 8048ed3:	85 d2                	test   %edx,%edx // 如果等于0
 8048ed5:	74 3a                	je     8048f11 <fun7+0x4a>
 8048ed7:	8b 1a                	mov    (%edx),%ebx
 8048ed9:	39 cb                	cmp    %ecx,%ebx // 如果ebx=36大于ecx
 8048edb:	7f 21                	jg     8048efe <fun7+0x37>
 8048edd:	b8 00 00 00 00       	mov    $0x0,%eax
 8048ee2:	39 cb                	cmp    %ecx,%ebx // 如果最开始输入的数字等于此时的ebx
...
08048f18 <secret_phase>:
 ...
 8048f2e:	89 c3                	mov    %eax,%ebx // 将输入转化为10进制,并存放在ebx中
 8048f30:	8d 40 ff             	lea    -0x1(%eax),%eax // eax--
 8048f33:	83 c4 10             	add    $0x10,%esp
 8048f36:	3d e8 03 00 00       	cmp    $0x3e8,%eax // 如果eax比1000大就爆炸
 ...
 8048f4e:	83 f8 06             	cmp    $0x6,%eax // eax要等于6,否则爆炸
